 ...
 *    p_max             lag found
 */
/* Double the Lag_max code with different defines and names */
static Word16 VAD1Lag_max(
#if 0 /* ifdef VAD2 */
		       Float32 corr[], Float32 sig[], Word16 L_frame,
		       Word32 lag_max, Word32 lag_min, Float32 *cor_max,
		       Word32 dtx, Float32 *rmax, Float32 *r0 )
#else
		       vad1State *vadSt, Float32 corr[], Float32 sig[], Word16
...
static Word16 VAD2Lag_max(
#if 1 /* ifdef VAD2 */
		       Float32 corr[], Float32 sig[], Word16 L_frame,
		       Word32 lag_max, Word32 lag_min, Float32 *cor_max,
		       Word32 dtx, Float32 *rmax, Float32 *r0 )
#else
		       vad2State *vadSt, Float32 corr[], Float32 sig[], Word16
      L_frame, Word32 lag_max, Word32 lag_min, Float32 *cor_max, Word32 dtx )
#endif
{
   Float32 max, T0;
 ...
 *    void
 */
/* Double the Pitch_ol code with different defines and names */
static Word32 VAD1Pitch_ol( enum Mode mode, vad1State *vadSt, Float32 signal[],
      Word32 pit_min, Word32 pit_max, Word16 L_frame, Word32 dtx, Word16 idx )
{
   ...
   j = pit_min << 2;
   p_max1 =
     VAD1Lag_max( corr_ptr, signal, L_frame, pit_max, j, &max1, dtx, &rmax1, &r01 );

   /* Maxima 2	*/
   i = j - 1;
   j = pit_min << 1;
   p_max2 = VAD1Lag_max( corr_ptr, signal, L_frame, i, j, &max2, dtx, &rmax2, &r02 );

   /* Maxima 3	*/
   i = j - 1;
   p_max3 =
     VAD1Lag_max( corr_ptr, signal, L_frame, i, pit_min, &max3, dtx, &rmax3, &r03 );
#else
   /* Find a maximum for each section.	*/
   /* Maxima 1	*/
   j = pit_min << 2;
   p_max1 = VAD1Lag_max( vadSt, corr_ptr, signal, L_frame, pit_max, j, &max1, dtx );

   /* Maxima 2 */
   i = j - 1;
   j = pit_min << 1;
   p_max2 = VAD1Lag_max( vadSt, corr_ptr, signal, L_frame, i, j, &max2, dtx );

   /* Maxima 3 */
   i = j - 1;
   p_max3 = VAD1Lag_max( vadSt, corr_ptr, signal, L_frame, i, pit_min, &max3, dtx );
...
static Word32 VAD2Pitch_ol( enum Mode mode, vad2State *vadSt, Float32 signal[],
      Word32 pit_min, Word32 pit_max, Word16 L_frame, Word32 dtx, Word16 idx )
{
   Float32 corr[PIT_MAX + 1];
   Float32 max1, max2, max3, p_max1, p_max2, p_max3;
   ...
   j = pit_min << 2;
   p_max1 =
     VAD2Lag_max( corr_ptr, signal, L_frame, pit_max, j, &max1, dtx, &rmax1, &r01 );

   /* Maxima 2	*/
   i = j - 1;
   j = pit_min << 1;
   p_max2 = VAD2Lag_max( corr_ptr, signal, L_frame, i, j, &max2, dtx, &rmax2, &r02 );

   /* Maxima 3	*/
   i = j - 1;
   p_max3 =
     VAD2Lag_max( corr_ptr, signal, L_frame, i, pit_min, &max3, dtx, &rmax3, &r03 );
#else
   /* Find a maximum for each section.	*/
   /* Maxima 1	*/
   j = pit_min << 2;
   p_max1 = VAD2Lag_max( vadSt, corr_ptr, signal, L_frame, pit_max, j, &max1, dtx );

   /* Maxima 2 */
   i = j - 1;
   j = pit_min << 1;
   p_max2 = VAD2Lag_max( vadSt, corr_ptr, signal, L_frame, i, j, &max2, dtx );

   /* Maxima 3 */
   i = j - 1;
   p_max3 = VAD2Lag_max( vadSt, corr_ptr, signal, L_frame, i, pit_min, &max3, dtx );
 ...
 *    p_max             lag found
 */
/* Double the Lag_max_wght code with different defines and names */
static Word32 VAD1Lag_max_wght( vad1State *vadSt, Float32 corr[], Float32 signal[],
...
static Word32 VAD2Lag_max_wght( vad2State *vadSt, Float32 corr[], Float32 signal[],
      Word32 old_lag, Word32 *cor_max, Word32 wght_flg, Float32 *gain_flg,
      Word32 dtx )
{
   Float32 t0, t1, max;
 ...
 */
/* Double the Pitch_ol_wgh code with different defines and names */
static Word32 VAD1Pitch_ol_wgh( Word32 *old_T0_med, Word16 *wght_flg, Float32 *ada_w,
      vad1State *vadSt, Float32 signal[], Word32 old_lags[], Float32 ol_gain_flg[],
      Word16 idx, Word32 dtx )
{
   ...
   p_max1 = VAD1Lag_max_wght( vadSt, corrPtr, signal, *old_T0_med,
...
static Word32 VAD2Pitch_ol_wgh( Word32 *old_T0_med, Word16 *wght_flg, Float32 *ada_w,
      vad2State *vadSt, Float32 signal[], Word32 old_lags[], Float32 ol_gain_flg[],
      Word16 idx, Word32 dtx )
{
   Float32 corr[PIT_MAX + 1];
#if 0 /* ifndef VAD2 */
   ...
   corrPtr = &corr[PIT_MAX];
   comp_corr( signal, L_FRAME_BY2, PIT_MAX, PIT_MIN, corrPtr );
   p_max1 = VAD2Lag_max_wght( vadSt, corrPtr, signal, *old_T0_med,
         &max1, *wght_flg, &ol_gain_flg[idx], dtx );
 ...
 *    void
 */
/* Double the ol_ltp code with different defines and names */
static void VAD1ol_ltp( enum Mode mode, vad1State *vadSt, Float32 wsp[], Word32 *T_op
      ...
      , Word32 *old_lags, Word32 dtx, Word16 idx )
{
   ...
   if ( ( mode == MR475 ) || ( mode == MR515 ) ) {
      *T_op = VAD1Pitch_ol( mode, vadSt, wsp, PIT_MIN, PIT_MAX, L_FRAME, dtx, idx );
   }
   else {
      if ( mode <= MR795 ) {
         *T_op = VAD1Pitch_ol( mode, vadSt, wsp, PIT_MIN, PIT_MAX, L_FRAME_BY2, dtx,
               idx );
      }
      else if ( mode == MR102 ) {
         *T_op = VAD1Pitch_ol_wgh( old_T0_med, wght_flg, ada_w, vadSt, wsp, old_lags,
            ol_gain_flg, idx, dtx );
      }
      else {
         *T_op = VAD1Pitch_ol( mode, vadSt, wsp, PIT_MIN_MR122, PIT_MAX, L_FRAME_BY2
               , dtx, idx );
      }
   }
}
static void VAD2ol_ltp( enum Mode mode, vad2State *vadSt, Float32 wsp[], Word32 *T_op
      , Float32 ol_gain_flg[], Word32 *old_T0_med, Word16 *wght_flg, Float32 *ada_w
      , Word32 *old_lags, Word32 dtx, Word16 idx )
{
   if ( mode != MR102 ) {
   ...
   if ( ( mode == MR475 ) || ( mode == MR515 ) ) {
      *T_op = VAD2Pitch_ol( mode, vadSt, wsp, PIT_MIN, PIT_MAX, L_FRAME, dtx, idx );
   }
   else {
      if ( mode <= MR795 ) {
         *T_op = VAD2Pitch_ol( mode, vadSt, wsp, PIT_MIN, PIT_MAX, L_FRAME_BY2, dtx,
               idx );
      }
      else if ( mode == MR102 ) {
         *T_op = VAD2Pitch_ol_wgh( old_T0_med, wght_flg, ada_w, vadSt, wsp, old_lags,
            ol_gain_flg, idx, dtx );
      }
      else {
         *T_op = VAD2Pitch_ol( mode, vadSt, wsp, PIT_MIN_MR122, PIT_MAX, L_FRAME_BY2
               , dtx, idx );
      }
   }
}
 ...
 *    void
 */
/* Double the cod_amr code with different defines and names */
static void VAD1cod_amr( cod_amrState *st, enum Mode mode, Float32 new_speech[],
      Word16 ana[], enum Mode *used_mode, Float32 synth[] )
{
   ...
   if ( st->dtx ) {
     ...
     vad_flag = vad2 (((vad1State*)(st->vadSt)), st->new_speech);
     vad_flag = vad2 (((vad1State*)(st->vadSt)), st->new_speech+80) || vad_flag;
#else
      /* Find VAD decision (option 1) */
      vad_flag = vad( ((vad1State*)(st->vadSt)), st->new_speech );
#endif
      /* force VAD on   */
      if ( *used_mode < 0 )
         vad_flag = 1;
...
#if 0 /* ifdef VAD2 */
   if (st->dtx) {
      ((vad1State*)(st->vadSt))->Rmax = 0.0;
      ((vad1State*)(st->vadSt))->R0 = 0.0;
   }
#endif
   ...
   for ( subfrNr = 0, i_subfr = 0; subfrNr < 2; subfrNr++, i_subfr +=
      ...
      if ( ( mode != MR475 ) && ( mode != MR515 ) ) {
         VAD1ol_ltp( mode, ((vad1State*)(((vad1State*)(st->vadSt)))), &st->wsp[i_subfr], &T_op[subfrNr], st->
   ...
   if ( ( mode == MR475 ) || ( mode == MR515 ) ) {
       ...
       */
      VAD1ol_ltp( mode, ((vad1State*)(((vad1State*)(st->vadSt)))), &st->wsp[0], &T_op[0], st->ol_gain_flg, &st->
...
#if 0 /* ifdef VAD2 */
   if (st->dtx) {
      LTP_flag_update(((vad1State*)(st->vadSt)), mode);
   }
#endif

#if 1 /* ifndef VAD2 */
   if ( st->dtx ) {
      vad_pitch_detection( ((vad1State*)(st->vadSt)), T_op );
   }
#endif
...
static void VAD2cod_amr( cod_amrState *st, enum Mode mode, Float32 new_speech[],
      Word16 ana[], enum Mode *used_mode, Float32 synth[] )
{
   /* LPC coefficients */
   Float32 A_t[( MP1 ) * 4];   /* A(z) unquantized for the 4 subframes */
...
#if 1 /* ifdef VAD2 */
     /* Find VAD decision (option 2) */
     vad_flag = vad2 (((vad2State*)(st->vadSt)), st->new_speech);
     vad_flag = vad2 (((vad2State*)(st->vadSt)), st->new_speech+80) || vad_flag;
#else
      /* Find VAD decision (option 1) */
      vad_flag = vad( ((vad2State*)(st->vadSt)), st->new_speech );
#endif
      /* force VAD on   */
      if ( *used_mode < 0 )
         vad_flag = 1;
...
#if 1 /* ifdef VAD2 */
   if (st->dtx) {
      ((vad2State*)(st->vadSt))->Rmax = 0.0;
      ((vad2State*)(st->vadSt))->R0 = 0.0;
   }
#endif
   ...
   for ( subfrNr = 0, i_subfr = 0; subfrNr < 2; subfrNr++, i_subfr +=
      ...
      /* Find open loop pitch lag for two subframes */
      if ( ( mode != MR475 ) && ( mode != MR515 ) ) {
         VAD2ol_ltp( mode, ((vad2State*)(((vad2State*)(st->vadSt)))), &st->wsp[i_subfr], &T_op[subfrNr], st->
               ol_gain_flg, &st->pitchOLWghtSt->old_T0_med, &st->pitchOLWghtSt->
               wght_flg, &st->pitchOLWghtSt->ada_w, st->old_lags, st->dtx,
               subfrNr );
      }
   ...
   if ( ( mode == MR475 ) || ( mode == MR515 ) ) {
       ...
       * search on 160 samples
       */
      VAD2ol_ltp( mode, ((vad2State*)(((vad2State*)(st->vadSt)))), &st->wsp[0], &T_op[0], st->ol_gain_flg, &st->
            pitchOLWghtSt->old_T0_med, &st->pitchOLWghtSt->wght_flg, &st->
            pitchOLWghtSt->ada_w, st->old_lags, st->dtx, 1 );
      T_op[1] = T_op[0];
   }
...
#if 1 /* ifdef VAD2 */
   if (st->dtx) {
      LTP_flag_update(((vad2State*)(st->vadSt)), mode);
   }
#endif

#if 0 /* ifndef VAD2 */
   if ( st->dtx ) {
      vad_pitch_detection( ((vad2State*)(st->vadSt)), T_op );
   }
#endif
 ...
 *    void
 */
/* Double the cod_amr_reset code with different defines and names */
static void VAD1cod_amr_reset( cod_amrState *s, Word32 dtx )
{
   Word32 i;
...
#if 0 /* ifdef VAD2 */
   /* reset vad1State */
   ((vad1State*)(s->vadSt))->pre_emp_mem = 0.0;
   ((vad1State*)(s->vadSt))->update_cnt = 0;
   ((vad1State*)(s->vadSt))->hyster_cnt = 0;
   ((vad1State*)(s->vadSt))->last_update_cnt = 0;
   for ( i = 0; i < NUM_CHAN; i++ ) {
     ((vad1State*)(s->vadSt))->ch_enrg_long_db[i] = 0.0;
     ((vad1State*)(s->vadSt))->ch_enrg[i] = 0.0;
     ((vad1State*)(s->vadSt))->ch_noise[i] = 0.0;
   }
   ((vad1State*)(s->vadSt))->Lframe_cnt = 0L;
   ((vad1State*)(s->vadSt))->tsnr = 0.0;
   ((vad1State*)(s->vadSt))->hangover = 0;
   ((vad1State*)(s->vadSt))->burstcount = 0;
   ((vad1State*)(s->vadSt))->fupdate_flag = 0;
   ((vad1State*)(s->vadSt))->negSNRvar = 0.0;
   ((vad1State*)(s->vadSt))->negSNRbias = 0.0;
   ((vad1State*)(s->vadSt))->R0 = 0.0;
   ((vad1State*)(s->vadSt))->Rmax = 0.0;
   ((vad1State*)(s->vadSt))->LTP_flag = 0;
#else
   /* reset vad1State */
   ((vad1State*)(s->vadSt))->oldlag_count = 0;
   ((vad1State*)(s->vadSt))->oldlag = 0;
   ((vad1State*)(s->vadSt))->pitch = 0;
   ((vad1State*)(s->vadSt))->tone = 0;
   ((vad1State*)(s->vadSt))->complex_high = 0;
   ((vad1State*)(s->vadSt))->complex_low = 0;
   ((vad1State*)(s->vadSt))->complex_hang_timer = 0;
   ((vad1State*)(s->vadSt))->vadreg = 0;
   ((vad1State*)(s->vadSt))->burst_count = 0;
   ((vad1State*)(s->vadSt))->hang_count = 0;
   ((vad1State*)(s->vadSt))->complex_hang_count = 0;

   /* initialize memory used by the filter bank */
   for ( i = 0; i < 3; i++ ) {
      ((vad1State*)(s->vadSt))->a_data5[i][0] = 0;
      ((vad1State*)(s->vadSt))->a_data5[i][1] = 0;
   }

   for ( i = 0; i < 5; i++ ) {
      ((vad1State*)(s->vadSt))->a_data3[i] = 0;
   }

   /* reset dtx_encState */
   /* initialize the rest of the memory */
   for ( i = 0; i < COMPLEN; i++ ) {
      ((vad1State*)(s->vadSt))->bckr_est[i] = NOISE_INIT;
      ((vad1State*)(s->vadSt))->old_level[i] = NOISE_INIT;
      ((vad1State*)(s->vadSt))->ave_level[i] = NOISE_INIT;
      ((vad1State*)(s->vadSt))->sub_level[i] = 0;
   }
   ((vad1State*)(s->vadSt))->best_corr_hp = CVAD_LOWPOW_RESET;
   ((vad1State*)(s->vadSt))->speech_vad_decision = 0;
   ((vad1State*)(s->vadSt))->complex_warning = 0;
   ((vad1State*)(s->vadSt))->sp_burst_count = 0;
   ((vad1State*)(s->vadSt))->corr_hp_fast = CVAD_LOWPOW_RESET;
#endif
...
static void VAD2cod_amr_reset( cod_amrState *s, Word32 dtx )
{
   Word32 i;
...
#if 1 /* ifdef VAD2 */
   /* reset vad2State */
   ((vad2State*)(s->vadSt))->pre_emp_mem = 0.0;
   ((vad2State*)(s->vadSt))->update_cnt = 0;
   ((vad2State*)(s->vadSt))->hyster_cnt = 0;
   ((vad2State*)(s->vadSt))->last_update_cnt = 0;
   for ( i = 0; i < NUM_CHAN; i++ ) {
     ((vad2State*)(s->vadSt))->ch_enrg_long_db[i] = 0.0;
     ((vad2State*)(s->vadSt))->ch_enrg[i] = 0.0;
     ((vad2State*)(s->vadSt))->ch_noise[i] = 0.0;
   }
   ((vad2State*)(s->vadSt))->Lframe_cnt = 0L;
   ((vad2State*)(s->vadSt))->tsnr = 0.0;
   ((vad2State*)(s->vadSt))->hangover = 0;
   ((vad2State*)(s->vadSt))->burstcount = 0;
   ((vad2State*)(s->vadSt))->fupdate_flag = 0;
   ((vad2State*)(s->vadSt))->negSNRvar = 0.0;
   ((vad2State*)(s->vadSt))->negSNRbias = 0.0;
   ((vad2State*)(s->vadSt))->R0 = 0.0;
   ((vad2State*)(s->vadSt))->Rmax = 0.0;
   ((vad2State*)(s->vadSt))->LTP_flag = 0;
#else
   /* reset vad2State */
   ((vad2State*)(s->vadSt))->oldlag_count = 0;
   ((vad2State*)(s->vadSt))->oldlag = 0;
   ((vad2State*)(s->vadSt))->pitch = 0;
   ((vad2State*)(s->vadSt))->tone = 0;
   ((vad2State*)(s->vadSt))->complex_high = 0;
   ((vad2State*)(s->vadSt))->complex_low = 0;
   ((vad2State*)(s->vadSt))->complex_hang_timer = 0;
   ((vad2State*)(s->vadSt))->vadreg = 0;
   ((vad2State*)(s->vadSt))->burst_count = 0;
   ((vad2State*)(s->vadSt))->hang_count = 0;
   ((vad2State*)(s->vadSt))->complex_hang_count = 0;

   /* initialize memory used by the filter bank */
   for ( i = 0; i < 3; i++ ) {
      ((vad2State*)(s->vadSt))->a_data5[i][0] = 0;
      ((vad2State*)(s->vadSt))->a_data5[i][1] = 0;
   }

   for ( i = 0; i < 5; i++ ) {
      ((vad2State*)(s->vadSt))->a_data3[i] = 0;
   }

   /* reset dtx_encState */
   /* initialize the rest of the memory */
   for ( i = 0; i < COMPLEN; i++ ) {
      ((vad2State*)(s->vadSt))->bckr_est[i] = NOISE_INIT;
      ((vad2State*)(s->vadSt))->old_level[i] = NOISE_INIT;
      ((vad2State*)(s->vadSt))->ave_level[i] = NOISE_INIT;
      ((vad2State*)(s->vadSt))->sub_level[i] = 0;
   }
   ((vad2State*)(s->vadSt))->best_corr_hp = CVAD_LOWPOW_RESET;
   ((vad2State*)(s->vadSt))->speech_vad_decision = 0;
   ((vad2State*)(s->vadSt))->complex_warning = 0;
   ((vad2State*)(s->vadSt))->sp_burst_count = 0;
   ((vad2State*)(s->vadSt))->corr_hp_fast = CVAD_LOWPOW_RESET;
#endif
